...
#define AT_DEBUG_CMP            0x02
#define AT_DEBUG                0 //(AT_DEBUG_SORT | AT_DEBUG_CMP)
...
#   define AT_DEV_USB_MASTER    "dev/udisk1110"
...
*/
static inline int
at_vcs_cmp(char *obj, at_vcs_t * a, at_vcs_t * b)
{
    int ret;

    if (at_vcs_is_good(a) && false==at_vcs_is_good(b)) {
        return 1;
    }
    else if (false==at_vcs_is_good(a) && at_vcs_is_good(b)) {
        return -1;
    }
...
static inline void
at_obj_cmp_dump(char *obj, int a, int b, int ret)
{
#if AT_DEBUG & AT_DEBUG_CMP
...
{
    int ret = at_vcs_cmp("kernel", at_kernel(a), at_kernel(b));

    at_obj_cmp_dump("kernel", a, b, ret);
...
{
    int ret = at_vcs_cmp("rootfs", at_rootfs(a), at_rootfs(b));

    at_obj_cmp_dump("rootfs", a, b, ret);
...
{
    int ret = __at_firmware_cmp(a, b);

    at_obj_cmp_dump("firmware", a, b, ret);
...
*/
static inline int
__at_obj_min(char *obj, int sort[], int count, int (*cmp)(int a, int b))
{
    int i, idx = 0;
...
#if AT_DEBUG & (AT_DEBUG_SORT | AT_DEBUG_CMP)
...
*/
static inline int
__at_obj_max(char *obj, int sort[], int count, int (*cmp)(int a, int b))
{
    int i, idx = 0;
...
{
    return __at_obj_min("kernel", sort, count, at_kernel_cmp);
}

static inline int
at_kernel_max(int sort[], int count)
{
    return __at_obj_max("kernel", sort, count, at_kernel_cmp);
}

static inline int
at_rootfs_min(int sort[], int count)
{
    return __at_obj_min("rootfs", sort, count, at_rootfs_cmp);
}

static inline int
at_rootfs_max(int sort[], int count)
{
    return __at_obj_max("rootfs", sort, count, at_rootfs_cmp);
}

static inline int
at_firmware_min(int sort[], int count)
{
    return __at_obj_min("firmware", sort, count, at_firmware_cmp);
}

static inline int
at_firmware_max(int sort[], int count)
{
    return __at_obj_max("firmware", sort, count, at_firmware_cmp);
}
